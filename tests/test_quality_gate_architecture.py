...
def test_installation_smoke_archives_active_outcome_before_commit_and_guards():
    ...
    assert (
        installation.index(finish) < installation.index(archive) < installation.index("git add .")
    )
    assert installation.index(archive) < installation.index("make check-ai-coverage-guard")
