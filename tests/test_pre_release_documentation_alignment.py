...
def test_current_report_is_aligned_and_generated_files_match():
    ...
    assert json.loads(Path("docs/reference/pre-release-documentation-alignment.json").read_text(encoding="utf-8")) == report
    assert Path("docs/reference/pre-release-documentation-alignment.md").read_text(encoding="utf-8") == render_markdown(report)


def test_marker_matching_handles_wrapped_text_and_missing_marker():
    assert marker_errors("example.md", "Evidence over\nSelf-Declaration", ("Evidence over Self-Declaration",)) == []
    assert marker_errors("example.md", "different text", ("required marker",)) == ["example.md: missing required marker: required marker"]
...
def test_bound_evidence_fails_closed_for_missing_drift_and_duplicate_paths(tmp_path):
    ...
    assert bound_evidence_errors(tmp_path, assessment) == ["Japanese bound evidence drift: bound.md"]
    duplicate = {"evidenceSource": {"files": [{"path": "bound.md", "sha256": checksum}, {"path": "bound.md", "sha256": checksum}]}}
    assert "Japanese bound path is duplicated: bound.md" in bound_evidence_errors(tmp_path, duplicate)
