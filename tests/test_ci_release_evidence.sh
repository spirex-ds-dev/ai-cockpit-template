...
jq -n --arg source "$head" --arg pub "$pub_digest" --arg candidate "$candidate_digest" --slurpfile evidence "$valid" '{schemaVersion:1,canonical:true,projections:{published:"release.json",candidate:"next-release.json"},state:"candidate_verified",releaseTag:"v0.5.34",sourceBinding:"exact",sourceCommit:$source,previousRelease:"v0.5.33",reservedTags:[],unavailableTags:[],evidenceStatus:"verified",evidenceBundleDigest:("a"*64),metadataDigests:{published:$pub,candidate:$candidate},ciEvidence:$evidence[0]}' > "$tmp/release/release-state.json"
