...
def test_acquire_reports_busy_when_the_same_lock_is_owned(tmp_path):
    lock_path = tmp_path / "target" / "quality" / "session.lock"
    with (
        quality_session_lock.acquire(lock_path, worktree=tmp_path),
        pytest.raises(quality_session_lock.QualitySessionBusy),
        quality_session_lock.acquire(lock_path, worktree=tmp_path),
    ):
        pass
...
def test_lock_runner_returns_child_status_and_busy_path_without_spawning(
...
def test_lock_runner_cli_parsing_and_main_delegate_to_resolved_paths(tmp_path, monkeypatch):
    lock = tmp_path / "lock"
    monkeypatch.setattr(
        "sys.argv",
        [
...
def test_lock_runner_cli_rejects_missing_command(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "sys.argv",
        ["quality_session_lock.py", "--lock", str(tmp_path / "lock")],
    )
    with pytest.raises(SystemExit, match="2"):
        quality_session_lock.parse_args()
