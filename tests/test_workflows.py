...
def test_compatibility_runs_lockfile_reproducibility_on_clean_runner():
    ...
    assert (
        "python -m pip install --disable-pip-version-check pip==25.2.1 pip-tools==7.6.0 typing-extensions==4.16.0"
        in lockfile
    )
...
def test_release_workflow_bootstraps_pinned_tool_before_lockfile_reproducibility():
    workflow = (ROOT / ".github" / "workflows" / "release.yml").read_text(encoding="utf-8")
    install = "python3 -m pip install --disable-pip-version-check pip==25.2.1 pip-tools==7.6.0 typing-extensions==4.16.0"
