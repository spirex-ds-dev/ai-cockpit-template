...
def test_committed_changes_since_base_remain_visible(tmp_path, monkeypatch):
    ...
    assert ai_common.changed_paths({"baseCommit": base, "baselineDirtyPaths": []}) == ["tracked.txt"]
...
def test_unchanged_preexisting_dirty_path_is_excluded(tmp_path, monkeypatch):
    ...
    assert ai_common.changed_paths({"baseCommit": base, "baselineDirtyPaths": dirty}) == ["tracked.txt"]
...
def test_deleting_preexisting_dirty_path_is_visible(tmp_path, monkeypatch):
    ...
    assert ai_common.changed_name_status({"baseCommit": base, "baselineDirtyPaths": dirty}) == [("D", "tracked.txt")]
...
def test_rename_checks_source_and_destination(tmp_path, monkeypatch):
    ...
    assert ai_common.changed_paths({"baseCommit": base, "baselineDirtyPaths": []}) == ["renamed.txt", "tracked.txt"]
...
def test_ci_base_commit_environment_overrides_contract(tmp_path, monkeypatch):
    ...
    assert ai_common.changed_paths({"baseCommit": "invalid", "baselineDirtyPaths": []}) == ["new.txt"]
