...
def test_repository_candidate_advances_past_reserved_release_failures():
    ...
    assert candidate["releaseTag"] == state["releaseTag"]
    assert candidate["basedOnReleaseTag"] == state["previousRelease"]
    assert state["previousRelease"] == published["releaseTag"]
    assert "v0.5.45" in state["reservedTags"]
    assert "v0.5.46" in state["reservedTags"]
    assert "v0.5.47" in state["reservedTags"]
    assert "v0.5.48" in state["reservedTags"]
    assert "v0.5.49" in state["reservedTags"]
    assert "v0.5.52" in state["reservedTags"]
    ...
    assert "v0.5.58" in state["reservedTags"]
    ...
    assert unavailable["v0.5.49"]["kind"] == "stable_release_invalid_public_distribution"
    assert "version.json" in unavailable["v0.5.49"]["reason"]
    ...
    assert unavailable["v0.5.48"]["kind"] == "stable_release_unverified"
    assert "source-bound release assets" in unavailable["v0.5.48"]["reason"]
    assert unavailable["v0.5.52"]["kind"] == "stable_release_invalid_public_distribution"
    assert "31390801439" in unavailable["v0.5.52"]["reason"]
    assert unavailable["v0.5.53"]["kind"] == "stable_release_invalid_public_distribution"
    assert (
        "6f00b72fc87fef3e67e36b0b9a315fd8e433d592acae1fb4e32c3c7fb6549cf6"
        in unavailable["v0.5.53"]["reason"]
    )
    ...
    assert unavailable["v0.5.60"]["kind"] == "stable_release_unverified"
    assert f'"releaseTag": "{published["releaseTag"]}"' in (ROOT / "release.json").read_text(
        encoding="utf-8"
    )
    assert check_release_state_consistency.check_repository(ROOT) == []
