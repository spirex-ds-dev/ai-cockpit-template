...
def test_current_facing_scan_rejects_a_non_object_payload() -> None:
    assert validate_current_facing_paths(ROOT, "not-a-registry") == ["registry must be an object"]


def test_current_facing_scan_rejects_invalid_paths_and_rules() -> None:
    payload = load_registry()
    payload["currentFacingScan"] = {
        "paths": [],
        "excludePrefixes": ["/outside-repository"],
        "prohibitedCommandChains": [
            "not-an-object",
            {"id": "", "pattern": "["},
            {"id": "duplicate", "pattern": ""},
            {"id": "duplicate", "pattern": "make quality"},
        ],
    }
    assert validate_current_facing_paths(ROOT, payload) == [
        "currentFacingScan paths must be a non-empty relative-path list",
        "currentFacingScan excludePrefixes must be a relative-path list",
        "currentFacingScan prohibitedCommandChains[0] must be an object",
        "currentFacingScan prohibitedCommandChains[1] id must be non-empty and unique",
        "currentFacingScan prohibitedCommandChains[1] pattern is invalid: unterminated character set at position 0",
        "currentFacingScan prohibitedCommandChains[2] pattern must be a non-empty string",
        "currentFacingScan prohibitedCommandChains[3] id must be non-empty and unique",
    ]
