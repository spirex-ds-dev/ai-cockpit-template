...
def test_source_bound_generated_evidence_policy_requires_exact_declared_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert source_bound_generated_evidence_policy_issues(contract, dependencies) == []

    contract["sourceBoundGeneratedEvidence"]["generatedPaths"] = [MATRIX_PATH]
    issues = source_bound_generated_evidence_policy_issues(contract, dependencies)
    assert "must declare exactly the canonical generated paths" in issues[0]


def test_source_bound_generated_evidence_change_rejects_omitted_or_unrelated_outputs(
    tmp_path: Path,
) -> None:
    write_file(tmp_path, "scripts/ai_finish.py")
    write_file(tmp_path, "tests/test_finish.py")
    ...
    missing = source_bound_generated_evidence_change_issues(
        contract, ["scripts/ai_finish.py", MATRIX_PATH], dependencies
    )
    assert "required generated path is absent from the diff" in missing[0]

    unrelated = source_bound_generated_evidence_change_issues(
        contract,
        [
            "scripts/ai_finish.py",
            *SOURCE_BOUND_GENERATED_DOCUMENTATION_PATHS,
    ...
    assert "does not authorize changed non-generated documentation" in unrelated[0]
