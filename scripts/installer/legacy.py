...
from installer.application import InstallationRequest
from installer.cli import prepare_request

CATALOG_NAME = "ai_installer_catalog.json"
CATALOG_PATH = Path(__file__).resolve().parents[1] / CATALOG_NAME
...
def main() -> int:
    args = parse_args()
    prepare_request(
        InstallationRequest(source=Path(args.source), target=Path(args.target), stack=args.stack)
    )
    return Installer(
