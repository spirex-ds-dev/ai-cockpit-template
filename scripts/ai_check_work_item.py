...
def validate_source_bound_generated_evidence(data: dict[str, Any]) -> list[str]:
    """Validate only the closed Contract shape; dependency relevance is preflight-derived."""
    ...
    issues: list[str] = []
    if set(policy) != {"mode", "generatedPaths"}:
    ...
    if policy.get("mode") != SOURCE_BOUND_GENERATED_EVIDENCE_MODE:
        issues.append(
            "sourceBoundGeneratedEvidence.mode must be " + SOURCE_BOUND_GENERATED_EVIDENCE_MODE
        )
    paths = policy.get("generatedPaths")
    if not isinstance(paths, list) or any(not non_empty_string(path) for path in paths):
        issues.append(
            "sourceBoundGeneratedEvidence.generatedPaths must be a list of non-empty strings"
        )
    elif set(paths) != set(SOURCE_BOUND_GENERATED_DOCUMENTATION_PATHS) or len(paths) != len(
        SOURCE_BOUND_GENERATED_DOCUMENTATION_PATHS
    ):
        issues.append(
...
def validate_contract(data: dict[str, Any], contract_path: str = "") -> list[str]:
    ...
    issues.extend(validate_source_bound_generated_evidence(data))
