...
def _governed_git(project_root: Path, *args: str) -> str:
    ...
    # The executable is absolute; callers use only fixed subcommands and list-form argv.
    result = subprocess.run(
...
def _rebase_onto(project_root: Path, target: str) -> None:
    executable = governed_git_executable()
    # target is a verified Git revision; execution is list-form with no shell.
    result = subprocess.run(
...
def _live_remote_head(project_root: Path, base_remote: str, base_branch: str) -> str:
    ...
    executable = governed_git_executable()
    # The remote is a configured name and branch is validated before this list-form invocation.
    result = subprocess.run(
        [
            executable,
            "ls-remote",
...
def synchronize_contract(
    ...
    *,
    ...
    candidate_contract["baseCommit"] = target
    candidate_contract["synchronizationHistory"] = [transition]
