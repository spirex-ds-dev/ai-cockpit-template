...
def _worktree_file_bytes(root: Path, relative_path: str) -> bytes:
    ...
    for part in Path(relative_path).parts:
        ...
        if current.is_symlink():
            raise ValueError(f"archive worktree member is not a regular file: {relative_path}")
    if not path.is_file():
        raise ValueError(f"archive worktree member is not a regular file: {relative_path}")
    ...
    with tarfile.open(fileobj=output, mode="w", format=tarfile.USTAR_FORMAT) as archive:
        for member in members:
            ...
            if member.isfile():
                ...
                content = (
                    _worktree_file_bytes(root, path)
                    if use_worktree
                    else subprocess.run(  # nosec B603 B607
...
def canonical_tar(root: Path, source_commit: str) -> bytes:
    """Serialize Git-selected committed paths using Python-owned stable metadata."""
...
def canonical_tar_from_worktree(root: Path, source_commit: str) -> bytes:
    """Serialize Git-selected paths using their current safe worktree bytes."""
