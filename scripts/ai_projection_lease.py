...
The lock deliberately outlives one ``ai-finish`` process.  A Work Item keeps
the lease until verified provider merge and lifecycle closure: another branch
must therefore refresh from the merged base before it writes the same tracked
projection (status, report, or archive index).
"""
...
def requires_lease(contract: object) -> bool:
    """Return whether this Contract explicitly opts into parallel projections.

    Historic and serial Work Items have no concurrency boundary.  They retain
    their established lifecycle behavior; only an explicitly declared parallel
    boundary activates the persistent branch-integrated projection lease.
    """
    return isinstance(contract, dict) and contract.get("concurrencyBoundary") is not None


class ProjectionLeaseError(RuntimeError):
    """Raised when another Work Item owns branch-integrated projections."""
...
def acquire(task: str, *, root: Path = PROJECT_ROOT) -> ProjectionLease:
    """Acquire or resume this task's persistent projection lease.

    Acquisition checks base freshness first.  This prevents a stale parallel
    branch from producing an archive index/report/status diff after another
    lease owner has merged.
    """
