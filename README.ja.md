...
Runtime のインストールは校正の完了を意味しません。初回校正は `configure_ai_cockpit` の 10 Stage を使用し、更新時は Impact Assessment を先に実行してから `recalibrate_ai_cockpit` を開始します。既存の Active Configuration は新しい Candidate の有効化が成功するまで有効です。検出範囲は宣言可能な既知リスクに対する決定的なカバレッジであり、すべての意味的危険や Agent の内部状態を分類するものではありません。既定の文書言語は日本語です。
