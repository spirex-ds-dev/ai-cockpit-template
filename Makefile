...
check-lockfile-reproducibility:
	@test -n "$(PYTHON_EXECUTABLE)" && test -x "$(PYTHON_EXECUTABLE)" || { echo "lockfile reproducibility check failed: PYTHON=$(PYTHON) did not resolve to an executable" >&2; exit 1; }; tmp=$$(mktemp -d); normalized=$$(mktemp -d); trap 'rm -rf "$$tmp" "$$normalized"' EXIT; cp requirements-dev.lock "$$tmp/"; (cd "$$tmp" && ln -s "$(CURDIR)/requirements-dev.in" requirements-dev.in && "$(PYTHON_EXECUTABLE)" -m piptools compile --no-upgrade --generate-hashes --allow-unsafe --output-file=requirements-dev.lock requirements-dev.in >/dev/null && sed -i.bak -E 's/^# This file is autogenerated by pip-compile with Python .*/# This file is autogenerated by pip-compile with canonical Python 3.10/' requirements-dev.lock && rm requirements-dev.lock.bak); awk '/^    # via/ { print "    # via"; skip=1; next } skip && /^    #   / { next } { skip=0; print }' requirements-dev.lock > "$$normalized/committed.lock"; awk '/^    # via/ { print "    # via"; skip=1; next } skip && /^    #   / { next } { skip=0; print }' "$$tmp/requirements-dev.lock" > "$$normalized/generated.lock"; cmp -s "$$normalized/generated.lock" "$$normalized/committed.lock" || { echo 'lockfile reproducibility check failed: regenerated requirements-dev.lock differs from the committed lockfile' >&2; exit 1; }
