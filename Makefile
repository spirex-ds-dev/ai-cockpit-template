...
check-lockfile-reproducibility:
	@set -e; test -n "$(PYTHON_EXECUTABLE)" && test -x "$(PYTHON_EXECUTABLE)" || { echo "lockfile reproducibility check failed: PYTHON=$(PYTHON) did not resolve to an executable" >&2; exit 1; }; tmp=$$(mktemp -d); trap 'rm -rf "$$tmp"' EXIT; cp requirements-dev.lock "$$tmp/"; (cd "$$tmp" && ln -s "$(CURDIR)/requirements-dev.in" requirements-dev.in && "$(PYTHON_EXECUTABLE)" -m piptools compile --no-upgrade --generate-hashes --allow-unsafe --output-file=requirements-dev.lock requirements-dev.in >/dev/null && sed -i.bak -E 's/^# This file is autogenerated by pip-compile with Python .*/# This file is autogenerated by pip-compile with canonical Python 3.10/' requirements-dev.lock && rm requirements-dev.lock.bak); cmp -s "$$tmp/requirements-dev.lock" requirements-dev.lock || { echo 'lockfile reproducibility check failed: regenerated requirements-dev.lock differs from the committed lockfile' >&2; exit 1; }
