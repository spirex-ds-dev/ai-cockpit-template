...
quality-full-owned:
	@set -eu; \
		...
		cleanup_project_test_receipt() { \
			status=$${1:-$$?}; \
			restore_project_test_receipt "$$status" || status=$$?; \
			exit $$status; \
		}; \
