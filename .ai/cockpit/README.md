...
After archive, the generated state is `no_active_work_item`. It means no active Contract/Summary pair. No-active status deliberately omits the file list and persists a deterministic clean marker; transient archive-time worktree changes are not serialized. Before the first archive-bundle commit, the matching start receipt is treated as part of that same transaction only when the current change set also contains a valid same-task archive Contract, Summary, manifest, and index update. An orphan receipt, historical-only or incomplete pair, malformed manifest, and every unrelated path remain fail closed. Commit the complete archived bundle first, then use `make check-ai-pr AI_BASE_COMMIT=<merge-base>` to validate the clean committed PR diff and archive ownership. `make repair-ai-status` only regenerates status for a valid zero- or one-active-pair lifecycle state; it does not repair ownership evidence.
