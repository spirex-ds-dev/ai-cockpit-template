...
| Governance Compression | Repository Truth を人間の意思決定に必要な最小シグナルへ圧縮する V2.5 の層。Summary を複製せず、決定に必要な証拠だけを提示する。 |
| Human Decision State | Cockpit が示す、人間が review / merge / block / investigate を判断するための状態。 |
| Governance Signals | Cockpit が出力する圧縮済みの判断信号。Intent、Acceptance、Unknowns、Verification、Guidelines、Checkpoints、Residual Risk などを含む。 |
...
| Cockpit Status | Active Work Item の状態を示す生成ファイル。V2.5 では Recommendation、Governance Signals、Evidence、Decision Drivers を含む。手動では編集しない。 |
| Repository Governance Layer | AI Cockpit の役割定義。Agent Runtime や Workflow Engine ではなく、リポジトリレベルのガバナンス、コンテキスト、検証、監査可能性を提供する。V2.5 ではこれに Human Decision State の圧縮が加わる。 |
