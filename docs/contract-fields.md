...
- **`destructiveChangePolicy.approvalEvidence.identityEvidence`**: `object`
  ...
  - 原則として `provider_verified` または `enterprise_verified` の完全な外部参照と対象 `scope` が必要です。`self_declared`、`repository_recorded`、または `approvedBy` だけの記録は高リスク承認として使用できません。
  - 例外の `direct_user_authorized` は、独立した Provider review が同一身份では不可能であり、ユーザーが直接、正確な削除範囲を許可した場合だけ使用できます。`provider: null`、直接指示の参照、`sha256:` digest、時刻、および `allowPatterns` と完全一致する `scope` が必要です。これは Provider/enterprise verification ではなく、そのように表示・報告してはなりません。
- **`rollbackNote`**: `string`
