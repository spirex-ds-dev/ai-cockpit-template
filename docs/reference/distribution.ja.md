...
変更は Pull Request を経由して `main` に入ります。`smoke` と `compatibility` は `main` への push でも実行されます。保守担当者は検証済みの `main` の SHA と新しいタグを指定して `.github/workflows/release.yml` を実行します。Premerge Finalizer は clean な候補 `HEAD` から開始し、Capability Truth、日本語評価、文書整合性レポートを生成します。この更新が管理対象レポート以外を変更した場合は fail closed します。その後、これらの追跡済み worktree bytes から `sourceTree` と `archiveSha256` を計算し、export-ignore のリリースメタデータと `installerDigest` を書き込みます。archive に含まれる証拠は、自身の archive identity を含む export-ignore メタデータを hash してはなりません。Workflow の Preflight は exact source の `install.sh` を独立に再ハッシュします。ワークフローは既存タグ、ソース SHA、smoke/compatibility の成功、`release.json` を確認してからタグと GitHub Release を作成します。
