...
The release job normally uses `ubuntu-latest`. An enterprise that enables a GitHub IP allow list and therefore blocks GitHub API calls from standard hosted-runner egress must configure the repository Actions variable `AI_COCKPIT_RELEASE_RUNNER` with a repository-authorized GitHub-hosted runner label that has a permitted static IP range, or with an authorized self-hosted runner label. The workflow then selects that label for the complete release job, including its exact-SHA evidence query. This is a routing prerequisite, not a gate bypass: a missing, unauthorized, or unavailable runner leaves the release blocked, and repositories without the variable retain the portable `ubuntu-latest` default.
SBOM and provenance release evidence must be generated with an explicit source commit (`--source-commit` or `SUPPLY_CHAIN_SOURCE_COMMIT`). The local release-tag fallback exists only for compatibility and never derives evidence identity from the current `HEAD`. The source-bound release archive is serialized by `scripts/release_archive.py`, which uses Git only to select export-ignored paths and Python to write stable tar/gzip bytes. When the workflow generates a runtime release digest, it projects that exact digest into the archive before serialization and byte-compares the extracted archive manifest with the Draft asset; a tag, source SHA, or correlation mismatch fails before publication. Preflight, freeze finalization, and the Hosted workflow must use this same builder; `git archive` bytes are not treated as portable evidence across runner Git versions.
