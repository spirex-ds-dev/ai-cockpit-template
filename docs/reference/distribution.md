...
The v0.5.42 release preparation supersedes the earlier quarantined attempts without rewriting any immutable tag and advances v0.5.43 as the distinct unpublished candidate. v0.5.42 may become public only after finalization marks its canonical archive binding verified and the workflow validates the real tagged Quick Install while the GitHub Release is still Draft.
