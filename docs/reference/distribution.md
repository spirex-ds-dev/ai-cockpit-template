...
The premerge finalizer starts from a clean candidate `HEAD`, materializes Capability Truth, the Japanese assessment, and documentation-alignment reports, and fails closed if that refresh changes any path outside its controlled report set. It then calculates `sourceTree` and `archiveSha256` from those exact tracked worktree bytes before writing export-ignored release metadata and the `installerDigest`; archive-included evidence must not hash an export-ignored field that contains its own archive identity. Hosted release preflight resolves the controlled source reference after PR merge, checks out that exact commit detached, and independently recalculates canonical content and the exact `install.sh` digest. If candidate and merged calculations disagree, publication stops before tag mutation and the process must be corrected before retry.
