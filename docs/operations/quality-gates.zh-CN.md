...
- Hosted smoke 明确拆为 `template-smoke`（唯一完整质量门所有者）、`installation-smoke` 和 `release-evidence` 三个 Job。三个 Job 独立启动，不重复调用完整质量图，并汇入一个使用 `always()` 的末端 `ci-evidence` Job；任何失败、取消或跳过都失败关闭。
