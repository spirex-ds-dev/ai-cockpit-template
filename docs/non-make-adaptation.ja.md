...
```makefile
# Makefile.ai.stack（typescript preset をベースに編集）
PROJECT_FORMAT_CHECK = npm run format:check
PROJECT_TEST = npm run test
PROJECT_LINT = npm run lint
```
...
csharp preset は `dotnet format`、`dotnet test`、`dotnet build` を前提とします。solution パスが非標準の場合は preset を編集します。
...
Coverage Guard と Scope Guard の glob は `.ai/guards/coverage_policy.yaml` と Project Profile でリポジトリ layout に合わせて調整します。preset の既定 pattern は出発点であり、monorepo 向けの完成形ではありません。

## 導入時の推奨手順

1. 既存 stack preset（`typescript`、`csharp` など）を `--stack` でインストールする。
