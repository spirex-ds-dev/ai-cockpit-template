...
`RFE-ISSUE-171`（多工作流发布证据集合，高）：v0.5.45 的 Provider release run `30604361511` 在 exact-SHA smoke 与 compatibility 均成功后，于 `Generate source-bound release evidence` fail closed，尚未创建 tag、Draft Release 或资产。根因是 evidence validator 把顶层 `requiredJobNames`（两个 workflow 的并集）错误地要求为每个单独 workflow run 的完整集合。独立 corrective `release-ci-evidence-multi-workflow-corrective-20260731` 必须改为验证所有 workflow-run 集合的精确并集，同时保留每个 run 的 job presence、success、Head SHA 和 conclusion 约束；以双 workflow 成功和 union mismatch 的机器回归证明。该 Work Item 完成 PR/Hosted CI/merge/closure 前不得重试 v0.5.45 发布。
...
| 文档对齐 | 历史 `pre-release-documentation-alignment-20260729` 停在旧基线且未交付；当前 `documentation-alignment-ruff016-binary-corrective-20260730` 从最新 `origin/main` 重建源绑定审计，并固定直接执行 Hosted Ruff 0.16 二进制。 |
| 发布前过期资产清理 | `pre-release-deprecated-assets-lifecycle-audit-20260730` 的 PR #498 因文档对齐生成物滞后而关闭，历史直接 main 提交仅作为事实保留，不能补写成生命周期完成。当前 replacement `pre-release-deprecated-assets-lifecycle-audit-replacement-20260731` 正在从文档对齐后的最新 main 重建审计与完整生命周期。 |
...
**RFE-RELEASE-FREEZE-FINAL-RECONCILIATION-20260731（最终 freeze 对齐，高，处理中）：** PR #530 合并后，旧 #527 生成的 freeze 仍绑定 `0b644...`，所以预检按设计拒绝当前 `main` 的 source tree、canonical archive 与 release digest。该独立工单只在 dedicated branch 上先 archive，再运行 `finalize-release-freeze-premerge` 生成当前 `origin/main` 的 export-ignored 发布投影，并以 evidence-only PR 的 Hosted、merge、closure 与 merged-main `check-release-preflight` 证明闭环。归档前场景门禁不得把“归档后才能发生的 finalizer”伪报为已验证；它仍是本工单的发布前验收与 PR 后证据。不得创建/移动 tag、GitHub Release 或资产，也不得改写旧 archive。

**RFE-RELEASE-EVIDENCE-MODEL-20260731（发布冻结循环，根因整改，高，处理中）：** PR #533 修复多 workflow 证据验证后改变了 archive-included 源树，使 #531 的 committed premerge freeze 再次过期。复核 #528 与 #529 发现它们均未合并、均基于过期的 #527 基线，且仅处理派生报告/工作树 archive 的相邻症状；不得直接合并。根因是同一 `check-release-preflight` 同时把历史 committed freeze 当作当前 main 的本地前置条件，又在 provider exact checkout 内重新 materialize 并验证真正的 source-bound runtime freeze。successor 必须把 repository readiness 与 strict runtime preflight 分离，并引入同 SHA、无 tag/Release/public asset 的 release rehearsal receipt；实际发布必须在任何不可变动作前验证该 receipt。后续源变更只失效 rehearsal SHA、要求重跑 rehearsal，不得再以“新 freeze Work Item”替代根因修复。#528/#529 仅在 successor Hosted CI 证明替代覆盖后关闭并删除分支，保留 PR 审计记录。
...
- `DEPENDABOT-444-PYTHON-BASELINE-001`（兼容性/发布前，高）：#444 的 `stevedore==5.9.0` 以 Python 3.10 真实 lockfile compile 失败，因该版本要求 Python >=3.11；这不是可通过修改锁文件绕过的问题。用户已明确授权把最低支持版本升至 Python 3.11。独立迁移必须先同步 runtime diagnostic、ruff/mypy、Python matrix、lockfile compiler、三语 README/quick-start/installation 文档和 release baseline，并以独立 PR/Hosted/closure 证明；不得在该迁移中混入 #444 的 pin，迁移关闭后才恢复 #444。
...
- `RFE-ISSUE-108`（Provider merge-state 不一致，高，处理中）：Python 3.11 PR #470 的 Head `117ec40e` 已作为 GitHub 验证签名 merge commit `dca16f0` 的第二父进入 `origin/main`，第一父为旧 base `6e19abce`；但 GitHub PR API 仍报告 `OPEN`、`mergedAt=null`、`mergeCommit=null`，且没有 merged timeline event 或 commit-to-PR association。普通 `ai-close-work-item` 因此正确拒绝 cleanup。根因边界是 provider merge transaction 的 partial success；现有审计 scope 无法证明上游触发。独立 corrective `provider-merge-state-recovery-20260730` 必须保留正常 `MERGED` 规则，新增显式且只读的异常证据评估：要求 OPEN PR、精确 PR Head/base/二亲 merge、GitHub signature、base reachability、所有 required hosted jobs 成功及显式确认；任何缺失/矛盾均在 receipt/cleanup 前失败。该 corrective 的 PR/Hosted/merge/closure 完成前，禁止重试 merge、force push、关闭 #470 或删除任何 #470 分支；之后以独立人类恢复决定执行下一步。
