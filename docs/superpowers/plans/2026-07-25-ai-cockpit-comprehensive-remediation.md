...
- `RFE-ISSUE-118`（流程/Summary 文档对齐证据，中）：总计划要求每个 Work Item 在 Summary 写入结构化 `documentationAlignment`，但当前 `ai_check_summary.py` 的允许字段集合拒绝该字段，导致“已完成对齐但无法按计划字段记录”的流程矛盾。当前 corrective 已将实际对齐范围与证据保存在允许的 changedFiles、userCorrectionSolidification 和 PLAN-DIRECTIVE-012 中，不删除事实也不放宽 validator；在恢复性能和发布前必须以独立 corrective 增加受校验的 documentationAlignment schema、反向证据检查和回归测试。
