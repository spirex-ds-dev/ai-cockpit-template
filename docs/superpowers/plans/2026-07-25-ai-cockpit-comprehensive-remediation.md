...
**RFE-RELEASE-FREEZE-DERIVED-EVIDENCE-20260731（发布冻结自引用，高，处理中）：** `release-v0-5-45-finalization-recovery-20260731` 的 PR #527 已完成 Hosted、合并与关闭，但合并后的 preflight 正确发现 finalizer 先从 committed candidate 计算 archive、再刷新 Capability Truth、日语评估和文档对齐。更深层地，Capability Truth 的 archive-included 行曾把 export-ignore 的 `release.json`（含 archive SHA）纳入字节证据，形成不可收敛的自引用。初始 `release-freeze-derived-evidence-order-20260731` 在本地归档后由 locked Ruff 发现 import lint 缺陷；该 archive 保持不可变且绝不合并。其 replacement `release-freeze-derived-evidence-order-replacement-20260731` 必须从当前 `origin/main` 完整重建：解除 self-reference，让 finalizer 仅刷新受控报告、拒绝意外路径或符号链接逃逸，并对生成后的 tracked worktree 计算 canonical bytes；然后在归档前通过 locked lint/check-ai-pr。必须以真实 no-ff merge/preflight 回归、PR、Hosted、merge、closure 证明新 freeze 可在合并后复算；replacement 和后续 premerge candidate 完整关闭前，v0.5.45 不得创建 tag、Release 或资产。
