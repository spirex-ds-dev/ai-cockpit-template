...
`RFE-ISSUE-084`（发布预检边界，高）：#401 重整后的本地专项验证显示，`check-release-distribution` 是发布后公共版本检查，读取历史 `release.json v0.5.42` 时会因最高公开 tag `v0.5.44` 而 fail closed；本地 `check-release-preflight` 也正确拒绝未在工单收口后 materialize 的 freeze、source tree 和 archive。未创建 tag 或 Release；后续必须先完成本恢复 PR 的 merge/close/分支清理，再由 provider 的 detached main 执行 finalizer、preflight 与公开发布。
...
`RFE-ISSUE-073`（发布恢复/版本序列，高）：#401 的 Draft asset verification 修复已归档但未合并；其 CI 将已创建但仍为 Draft 的 immutable `v0.5.44` tag 误称为“accepted previous release”，同时 `next-release.json` 仍为 v0.5.44，导致 corrective PR 无法通过自身的 release-preparation gate。用户授权在同一 PR 建立有记录的恢复 Work Item。流程已固化为：失败 Draft tag 记录为 canonical `reservedTags`，不改变 `release.json`/`basedOnReleaseTag` 的已发布 v0.5.42 基线；候选必须推进到最高 reserved tag 后的唯一下一 patch（本次 v0.5.45）；`check-release-recovery-policy` 与回归测试强制该规则；完成恢复 Work Item 的 archive/PR/merge/close/分支清理后才重试 provider 发布。不得静默把 archived Work Item 的 scope 扩大或复用失败 tag。
