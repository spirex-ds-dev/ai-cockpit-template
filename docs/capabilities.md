...
| Implementation Knowledge projection | `adopter_installed` | Keep validated implementation records and indexes derived from completed evidence. | The normal path uses dependency mapping to refresh affected records only; missing or untrusted mapping triggers explicit full rebuild/revalidation or fail-closed behavior, which can cost more as history grows. | [Knowledge guide](reference/implementation-knowledge.md) |
