...
Public `v0.5.7` includes the auditable adoption bootstrap. The first installation PR can generate its own bounded Contract/Summary pair and pass complete `check-ai-pr` ownership after the documented finish and commit steps.
...
The documented release is defined in `release.json`. Public `v0.5.7` supports caller-provided `AI_COCKPIT_TEMPLATE_SHA256` verification and fails before extraction when the downloaded archive digest differs.
...
Without `--update-makefile`, the installer does not modify the host `Makefile`; it writes separate `Makefile.ai` and `Makefile.ai.stack` files. The recommended command above does pass `--update-makefile`, so it appends `include Makefile.ai`. Public v0.5.7 validates reserved `ai-cockpit-*` targets before writing and exposes namespaced project-quality targets.

Other conservative defaults:

- Public v0.5.7 uses namespaced `ai-cockpit-*` recipes to avoid replacing common host targets.
