...
After a successful closure, the executor must send the command's **Human completion report** directly to the human conversation. Lead with the delivered Work Item, verified local evidence, merged PR/commit, cleanup, base synchronization, residual-risk status, and next-work guidance. Add the exact-head Hosted CI conclusion that the executor independently verified before merge. Outcome and Closure Receipt paths are optional audit links after that summary; never make the human open files to learn the outcome, and never infer Hosted CI success merely from a merged PR.
