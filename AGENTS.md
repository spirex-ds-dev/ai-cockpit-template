...
The canonical order is: latest remote base → dedicated Work Item branch → implement → `ai-finish` (verification and direct pre-archive report while still active) → human confirmation → archive → push branch → PR → merge PR → `make ai-close-work-item` → synchronize and clean the local base. The executor must send the pre-archive report directly to the human conversation and wait for confirmation before archive, push, or PR creation. This timing is identical for manual PR creation and separately permitted automation. Do not merge the feature branch into local `main` before the PR; that creates local commits that are not on `origin/main`. Do not use a PR merge option or provider setting that deletes the Work Item branch before `ai-close-work-item` can identify it. Branch deletion belongs to lifecycle closure.
